 ...
 */
export async function parseW54SnapshotHtmlFromUrl(url) {
 ...
 */
function parseW54SnapshotHtml(html, source) {
    ...
    $tables.each((_tableIdx, tableEl) => {
        ...
        const leagueParts = titleButtons.map((btn) => textTrim($(btn).text())).filter(Boolean);
        ...
        if ($matchRows.length === 0) {
            $matchRows = $table.find("tr").filter((_i, row) => {
                const $row = $(row);
                // Skip header rows
                if ($row.hasClass("LinesGroup_header"))
                    return false;
...
export async function parseW54SnapshotHtmlFromFile(filePath) {
