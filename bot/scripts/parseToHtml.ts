        ...
        try {
            ...
            try {
                ...
                const submitClicked = await page.evaluate(() => {
                    ...
                    const submitBtn = document.querySelector('button.LoginForm_submitButton__FvjWH') as HTMLElement;
                    if (submitBtn && !submitBtn.disabled) {
                        submitBtn.click();
                        return true;
                    }
                    // Пробуем найти кнопку submit
                    const submitBtn2 = document.querySelector('button[type="submit"]') as HTMLElement;
                    ...
                    const buttons = Array.from(document.querySelectorAll('button'));
                    const enterBtn = buttons.find(btn => btn.textContent?.includes('Войти') && !(btn as HTMLButtonElement).disabled);
                    if (enterBtn) {
